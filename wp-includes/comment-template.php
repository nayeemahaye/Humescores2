...
function get_comment_date( $d = 'l, F j, Y', $comment_ID = 0 ) {
 ...
 */
function get_comment_time( $t = '', $gmt = false, $translate = true ) {
	$comment = get_comment();

	$comment_date = $gmt ? $comment->comment_date_gmt : $comment->comment_date;
	if ( '' == $t ) {
		$date = mysql2date(get_option('time_format'), $comment_date, $translate);
                 debug_to_console( "no T format" );
        }
	else {
		$date = mysql2date($t, $comment_date, $translate);
                 debug_to_console( "yes T format" );
        }
	 ...
	 * @param WP_Comment $comment   The comment object.
	 */
//	return apply_filters( 'get_comment_time', $date, $t, $gmt, $translate, $comment );
}
