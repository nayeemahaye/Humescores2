...
<header class="archive-header active">
	<h1><?php esc_html_e( 'Hello', 'calafate' ); ?></h1>
	<h3><?php echo esc_html( $current_user->display_name ); ?></h3>
	<h4><?php echo sprintf( esc_attr__( '(not %1$s? %2$sSign out%3$s)', 'calafate' ), esc_html( $current_user->display_name ), '<a href="' . esc_url( wc_get_endpoint_url( 'customer-logout', '', wc_get_page_permalink( 'myaccount' ) ) ) . '">', '</a>' ); ?></h4>
</header>

<p>
	<?php
		echo sprintf( esc_attr__( 'From your account dashboard you can view your %1$srecent orders%2$s, manage your %3$sshipping and billing addresses%2$s and %4$sedit your password and account details%2$s.', 'calafate' ), '<a href="' . esc_url( wc_get_endpoint_url( 'orders' ) ) . '">', '</a>', '<a href="' . esc_url( wc_get_endpoint_url( 'edit-address' ) ) . '">', '<a href="' . esc_url( wc_get_endpoint_url( 'edit-account' ) ) . '">' );
	?>
</p>
